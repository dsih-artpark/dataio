...
def parse_args():
    ...
    parser.add_argument('-u','--add-test-users', action='store_true', help='Add test users to the database')
    parser.add_argument('-g', '--add-groups', action='store_true', help='Add groups to the database')
    parser.add_argument('-r', '--add-resource-groups', action='store_true', help='Add resource groups to the database')
    parser.add_argument('-a', '--add-user-permissions', action='store_true', help='Add user permissions to the database')
...
def add_test_users():
    conn = connect_to_db()
    cur = conn.cursor()
    test_users = [generate_user("admin@artpark.in"),
    ...
    for user in test_users:
        cur.execute(
            """INSERT INTO users (email, key, is_group) VALUES (%s, %s, %s)""",
            (user['email'], user['key'], user['is_group']))
...
def add_groups():
    ...
    group_mapping = {
        ...
        "livestock_group@artpark.in": ["analyst@artpark.in", "external_collaborator@psu.edu"]
    }
...
def add_resource_groups():
    ...
    for resource in resource_group['resources']:
        cur.execute(
            ...
            (resource_group['group_name'], resource['resource_id'], resource['resource_type'], resource['resource_json'])
        )
    conn.commit()
    ...
    for user_permission in user_permissions:
        cur.execute(
            """INSERT INTO user_permissions (user_email, resource_type, resource_id, permission) 
               VALUES (%s, %s, %s, %s)""",
            (user_permission['user_email'], user_permission['resource_type'], 
             user_permission['resource_id'], user_permission['permission'])
        )
    conn.commit()
